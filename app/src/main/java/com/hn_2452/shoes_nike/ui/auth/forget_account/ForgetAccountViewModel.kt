...
class ForgetAccountViewModel @Inject constructor(
    ...
    fun findPasswordByEmail(email: String) = liveData(handleEx(mContext)) {
        try {
    ...
    fun sendCodeAgain() = liveData(handleEx(mContext)) {
        try {
    ...
    fun changePasswordByCode(authCode: String?, newPassword: String?, checkerPass: String) =
        liveData(
            handleEx(mContext)
        ) {
            try {
                ...
                emit(Resource.loading())
                emit(
                    mAuthRepository.changePasswordByAuthCode(
                        mCurrentEmail!!,
                        authCode,
                        newPassword
                    )
