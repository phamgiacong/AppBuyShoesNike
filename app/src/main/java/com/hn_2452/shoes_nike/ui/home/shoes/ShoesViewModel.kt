...
class ShoesViewModel @Inject constructor(
    ...
    fun buyNow() = liveData {
        if (mCurrentShoes.value == null) {
            Toast.makeText(mContext, mContext.getString(R.string.application_not_ready), Toast.LENGTH_SHORT).show()
            return@liveData
        }

        if (mSelectedColor.isNullOrEmpty()) {
            Toast.makeText(mContext, mContext.getString(R.string.request_select_color), Toast.LENGTH_SHORT).show()
            return@liveData
        }

        if (mSelectedSize == -1) {
            Toast.makeText(mContext, mContext.getString(R.string.request_select_size), Toast.LENGTH_SHORT).show()
            return@liveData
        }

        emit(OrderDetail("", "", mCurrentShoes.value!!, mSelectedNumber, mSelectedColor!!, mSelectedSize))
    }
